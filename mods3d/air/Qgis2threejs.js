...
Q3D_air.ua = window.navigator.userAgent.toLowerCase();
Q3D_air.isIE = (Q3D_air.ua.indexOf("msie") != -1 || Q3D_air.ua.indexOf("trident") != -1);
Q3D_air.isTouchDevice = ("ontouchstart" in window);

Q3D_air.$ = function (elementId) {
  return document.getElementById(elementId);
};

/*
Q3D_air.Group -> THREE.Group -> THREE.Object3D
*/
Q3D_air.Group = function () {
  THREE.Group.call(this);
};

Q3D_air.Group.prototype = Object.create(THREE.Group.prototype);
Q3D_air.Group.prototype.constructor = Q3D_air.Group;

Q3D_air.Group.prototype.add = function (object) {
  THREE.Group.prototype.add.call(this, object);
  object.updateMatrixWorld();
};

Q3D_air.Group.prototype.clear = function () {
...
/*
Q3D_air.Scene -> THREE.Scene -> THREE.Object3D
...
Q3D_air.Scene = function () {
  ...
  this.lightGroup = new Q3D_air.Group();
  this.add(this.lightGroup);

  this.labelConnectorGroup = new Q3D_air.Group();
  this.add(this.labelConnectorGroup);

  this.labelRootElement = null;
};

Q3D_air.Scene.prototype = Object.create(THREE.Scene.prototype);
Q3D_air.Scene.prototype.constructor = Q3D_air.Scene;

Q3D_air.Scene.prototype.add = function (object) {
...
Q3D_air.Scene.prototype.loadJSONObject = function (jsonObject) {
  if (jsonObject.type == "scene") {
    ...
    if (layer === undefined) {
      ...
      if (type == "dem") layer = new Q3D_air.DEMLayer();
      else if (type == "point") layer = new Q3D_air.PointLayer();
      else if (type == "line") layer = new Q3D_air.LineLayer();
      else if (type == "polygon") layer = new Q3D_air.PolygonLayer();
      else {
...
Q3D_air.Scene.prototype.buildLights = function (lights) {
...
Q3D_air.Scene.prototype.buildDefaultLights = function () {
  this.buildLights(Q3D_air.Config.lights);
};

Q3D_air.Scene.prototype.requestRender = function () {
  this.dispatchEvent({type: "renderRequest"});
};

Q3D_air.Scene.prototype.queryableObjects = function () {
  var objs = [];
...
Q3D_air.Scene.prototype.toMapCoordinates = function (x, y, z) {
...
Q3D_air.Scene.prototype.toLocalCoordinates = function (x, y, z, isProjected) {
...
Q3D_air.Scene.prototype._rotatePoint = function (point, degrees, origin) {
...
Q3D_air.Scene.prototype.adjustZShift = function () {
  // initialize
...
(function () {
  ...
  appAIR.addEventListener = function (type, listener, prepend) {
  ...
  appAIR.init = function (container) {
    appAIR.container = container;
    appAIR.running = false;        // if true, animation loop is continued.

    // URL parameters
    appAIR.urlParams = appAIR.parseUrlParameters();
    if ("popup" in appAIR.urlParams) {
      // open popup window
      var c = window.location.href.split("?");
      window.open(c[0] + "?" + c[1].replace(/&?popup/, ""), "popup", "width=" + appAIR.urlParams.width + ",height=" + appAIR.urlParams.height);
      appAIR.popup.show("Another window has been opened.");
      return;
    }

    if (appAIR.urlParams.width && appAIR.urlParams.height) {
      // set container size
      container.style.width = appAIR.urlParams.width + "px";
      container.style.height = appAIR.urlParams.height + "px";
    }

    appAIR.width = container.clientWidth;
    appAIR.height = container.clientHeight;

    var bgcolor = Q3D_air.Config.bgColor;
    if (bgcolor === null) container.classList.add("sky");

    // WebGLRenderer
    appAIR.renderer = new THREE.WebGLRenderer({alpha: true, antialias: true});
    appAIR.renderer.setSize(appAIR.width, appAIR.height);
    appAIR.renderer.setClearColor(bgcolor || 0, (bgcolor === null) ? 0 : 1);
    appAIR.container.appendChild(appAIR.renderer.domElement);

    // set viewpoint if specified by URL parameters
    var vars = appAIR.urlParams;
    if (vars.cx !== undefined) Q3D_air.Config.viewpoint.pos = {x: parseFloat(vars.cx), y: parseFloat(vars.cy), z: parseFloat(vars.cz)};
    if (vars.tx !== undefined) Q3D_air.Config.viewpoint.lookAt = {x: parseFloat(vars.tx), y: parseFloat(vars.ty), z: parseFloat(vars.tz)};

    // camera
    appAIR.buildCamera(Q3D_air.Config.orthoCamera);
    ...
    //  controls = new THREE.OrbitControls(appAIR.camera, appAIR.renderer.domElement);
    //  controls.enableKeys = false;

    //  var t = Q3D_air.Config.viewpoint.lookAt;
    ...
    //    offset.y = 0;
    //    offset.normalize();
    //    offset.multiplyScalar(-2 * delta * targetDistance / appAIR.renderer.domElement.clientHeight);
    ...
    //appAIR.controls = controls;
    //appAIR.controls.update();

    //appAIR.labelVisible = Q3D_air.Config.label.visible;

    // root element of labels
    //appAIR.scene.labelRootElement = document.getElementById("labels");
    //appAIR.scene.labelRootElement.style.display = (appAIR.labelVisible) ? "block" : "none";

    // create a marker for queried point
    //var opt = Q3D_air.Config.qmarker;
    //appAIR.queryMarker = new THREE.Mesh(new THREE.SphereBufferGeometry(opt.r),
    //                                 new THREE.MeshLambertMaterial({color: opt.c, opacity: opt.o, transparent: (opt.o < 1)}));

    //appAIR.highlightMaterial = new THREE.MeshLambertMaterial({emissive: 0x999900, transparent: true, opacity: 0.5});
    //if (!Q3D_air.isIE) appAIR.highlightMaterial.side = THREE.DoubleSide;    // Shader compilation error occurs with double sided material on IE11

    //appAIR.selectedObject = null;
    //appAIR.highlightObject = null;

    //appAIR.modelBuilders = [];
    //appAIR._wireframeMode = false;

    //// add event listeners
    //appAIR.addEventListener("sceneLoaded", function () {
    //  if (Q3D_air.Config.autoZShift) {
    //    appAIR.scene.adjustZShift();
    //  }
    //  appAIR.render();
    //}, true);

    //window.addEventListener("keydown", appAIR.eventListener.keydown);
    //window.addEventListener("resize", appAIR.eventListener.resize);

    //appAIR.renderer.domElement.addEventListener("mousedown", appAIR.eventListener.mousedown);
    //appAIR.renderer.domElement.addEventListener("mouseup", appAIR.eventListener.mouseup);

    //var e = Q3D_air.$("closebtn");
    //if (e) e.addEventListener("click", appAIR.closePopup);
  };

  appAIR.parseUrlParameters = function () {
    var p, vars = {};
  ...
  appAIR.initLoadingManager = function () {
    if (appAIR.loadingManager) {
      appAIR.loadingManager.onLoad = appAIR.loadingManager.onProgress = appAIR.loadingManager.onError = undefined;
    }

    appAIR.loadingManager = new THREE.LoadingManager(function () {
      // onLoad
      appAIR.loadingManager.isLoading = false;
    ...
    appAIR.loadingManager.onStart = function () {
      appAIR.loadingManager.isLoading = true;
    };

    appAIR.loadingManager.isLoading = false;
  };

  appAIR.initLoadingManager();

  appAIR.loadFile = function (url, type, callback) {

    var loader = new THREE.FileLoader(appAIR.loadingManager);
    loader.setResponseType(type);

    var onError = function (e) {
      if (location.protocol == "file:") {
        appAIR.popup.show("This browser doesn't allow loading local files via Ajax. See <a href='https://github.com/minorua/Qgis2threejs/wiki/Browser-Support'>plugin wiki page</a> for details.", "Error", true);
      }
    };
  ...
  appAIR.loadJSONObject = function (jsonObject) {
    appAIR.scene.loadJSONObject(jsonObject);
  };

  appAIR.loadJSONFile = function (url, callback) {
    appAIR.loadFile(url, "json", function (obj) {
      appAIR.loadJSONObject(obj);
      if (callback) callback(obj);
    });
  };

  appAIR.loadSceneFile = function (url, callback) {
    var ext = url.split(".").pop();
    if (ext == "json") appAIR.loadJSONFile(url, callback);
    else if (ext == "js") {
  ...
  appAIR.loadTextureFile = function (url, callback) {
    return new THREE.TextureLoader(appAIR.loadingManager).load(url, callback);
  };

  appAIR.loadModelFile = function (url, callback) {
    var loader,
        ext = url.split(".").pop();
    if (ext == "dae") {
      loader = new THREE.ColladaLoader(appAIR.loadingManager);
    }
    else if (ext == "gltf" || ext == "glb") {
      loader = new THREE.GLTFLoader(appAIR.loadingManager);
    }
    else {
    ...
    appAIR.loadingManager.itemStart("M" + url);

    loader.load(url, function (model) {
      if (callback) callback(model);
      appAIR.loadingManager.itemEnd("M" + url);
    },
    undefined,
    function (e) {
      console.log("Failed to load model: " + url);
      appAIR.loadingManager.itemError("M" + url);
    });
  };


  appAIR.loadModelData = function (data, ext, resourcePath, callback) {

    if (ext == "dae") {
      var model = new THREE.ColladaLoader(appAIR.loadingManager).parse(data, resourcePath);
      if (callback) callback(model);
    }
    else if (ext == "gltf" || ext == "glb") {
      new THREE.GLTFLoader(appAIR.loadingManager).parse(data, resourcePath, function (model) {
        if (callback) callback(model);
      }, function (e) {
  ...
  appAIR.setCanvasSize = function (width, height) {
    appAIR.width = width;
    appAIR.height = height;
    appAIR.camera.aspect = width / height;
    appAIR.camera.updateProjectionMatrix();
    appAIR.renderer.setSize(width, height);
  };

  appAIR.buildCamera = function (is_ortho) {
    if (is_ortho) {
    ...
    var v = Q3D_air.Config.viewpoint,
        p = v.pos,
        t = v.lookAt;
    appAIR.camera.position.set(p.x, p.y, p.z);
    appAIR.camera.lookAt(t.x, t.y, t.z);
  };
  ...
  appAIR.currentViewUrl = function () {
    var c = appAIR.camera.position, t = appAIR.controls.target;
  ...
  // enable the controls
  appAIR.start = function () {
    if (appAIR.controls) appAIR.controls.enabled = true;
  };

  appAIR.pause = function () {
    appAIR.running = false;
    if (appAIR.controls) appAIR.controls.enabled = false;
  };

  appAIR.resume = function () {
    if (appAIR.controls) appAIR.controls.enabled = true;
  };

  appAIR.startAnimation = function () {
    appAIR.running = true;
    appAIR.animate();
  };

  appAIR.stopAnimation = function () {
    appAIR.running = false;
  };

  // animation loop
  appAIR.animate = function () {
    if (appAIR.running) requestAnimationFrame(appAIR.animate);
    appAIR.render(true);
  };

  appAIR.render = function (updateControls) {
    if (updateControls) appAIR.controls.update();
  ...
  appAIR.saveCanvasImage = function (width, height, fill_background, saveImageFunc) {
    ...
    var old_size;
    if (width && height) {
      old_size = [appAIR.width, appAIR.height];
      appAIR.setCanvasSize(width, height);
    }
...
Q3D_air.Material
*/
Q3D_air.Material = function () {
  this.loaded = false;
};

Q3D_air.Material.prototype = {

  constructor: Q3D_air.Material,
  ...
  loadJSONObject: function (jsonObject, callback) {
    ...
    if (m.ds && !Q3D_air.isIE) opt.side = THREE.DoubleSide;
    ...
    if (m.image !== undefined) {
      ...
      if (m.image.url !== undefined) {
        opt.map = Q3D_air.application.loadTextureFile(m.image.url, function () {
    ...
    if (m.type == Q3D_air.MaterialType.MeshLambert) {
      this.mtl = new THREE.MeshLambertMaterial(opt);
    }
    else if (m.type == Q3D_air.MaterialType.MeshPhong) {
      this.mtl = new THREE.MeshPhongMaterial(opt);
    }
    else if (m.type == Q3D_air.MaterialType.MeshToon) {
      this.mtl = new THREE.MeshToonMaterial(opt);
    }
    else if (m.type == Q3D_air.MaterialType.Point) {
      opt.size = m.s;
      this.mtl = new THREE.PointsMaterial(opt);
    }
    else if (m.type == Q3D_air.MaterialType.LineBasic) {
      this.mtl = new THREE.LineBasicMaterial(opt);
    }
    else if (m.type == Q3D_air.MaterialType.LineDashed) {
      opt.dashSize = Q3D_air.Config.line.dash.dashSize;
      opt.gapSize = Q3D_air.Config.line.dash.gapSize;
  ...
  type: function () {
    if (this.mtl instanceof THREE.MeshLambertMaterial) return Q3D_air.MaterialType.MeshLambert;
    if (this.mtl instanceof THREE.MeshPhongMaterial) return Q3D_air.MaterialType.MeshPhong;
    if (this.mtl instanceof THREE.LineBasicMaterial) return Q3D_air.MaterialType.LineBasic;
    if (this.mtl instanceof THREE.LineDashedMaterial) return Q3D_air.MaterialType.LineDashed;
    if (this.mtl instanceof THREE.SpriteMaterial) return Q3D_air.MaterialType.Sprite;
    if (this.mtl === undefined) return undefined;
    if (this.mtl === null) return null;
    return Q3D_air.MaterialType.Unknown;
  },
...
Q3D_air.Materials
*/
Q3D_air.Materials = function () {
  this.materials = [];
};

Q3D_air.Materials.prototype = Object.create(THREE.EventDispatcher.prototype);
Q3D_air.Materials.prototype.constructor = Q3D_air.Materials;

// material: instance of Q3D_air.Material object or THREE.Material-based object
Q3D_air.Materials.prototype.add = function (material) {
  if (material instanceof Q3D_air.Material) this.materials.push(material);
  else {
    var mtl = new Q3D_air.Material();
    mtl.set(material);
    this.materials.push(mtl);
  }
};

Q3D_air.Materials.prototype.get = function (index) {
  return this.materials[index];
};

Q3D_air.Materials.prototype.mtl = function (index) {
  return this.materials[index].mtl;
};

Q3D_air.Materials.prototype.loadJSONObject = function (jsonObject) {
...
Q3D_air.Materials.prototype.dispose = function () {
...
Q3D_air.Materials.prototype.addFromObject3D = function (object) {
  var mtls = [];
...
// opacity
Q3D_air.Materials.prototype.opacity = function () {
...
Q3D_air.Materials.prototype.setOpacity = function (opacity) {
  var material;
  for (var i = 0, l = this.materials.length; i < l; i++) {
...
// wireframe: boolean
Q3D_air.Materials.prototype.setWireframeMode = function (wireframe) {
  var material;
  for (var i = 0, l = this.materials.length; i < l; i++) {
    material = this.materials[i];
...
Q3D_air.DEMBlock
*/
Q3D_air.DEMBlock = function () {};

Q3D_air.DEMBlock.prototype = {

  constructor: Q3D_air.DEMBlock,
  ...
  loadJSONObject: function (obj, layer, callback) {
    ...
    // load material
    this.material = new Q3D_air.Material();
    ...
    if (grid.url !== undefined) {
      Q3D_air.application.loadFile(grid.url, "arraybuffer", function (buf) {
...
/*
Q3D_air.ClippedDEMBlock
*/
Q3D_air.ClippedDEMBlock = function () {};

Q3D_air.ClippedDEMBlock.prototype = {

  constructor: Q3D_air.ClippedDEMBlock,
  ...
  loadJSONObject: function (obj, layer, callback) {
    ...
    if (obj.geom.url !== undefined) {
      Q3D_air.application.loadFile(obj.geom.url, "json", function (obj) {
        buildGeometry(obj);
      });
    }
    else {    // WebKit Bridge
  ...
  buildSides: function (layer, parent, material, z0) {
    ...
    for (var i = 0, l = polygons.length; i < l; i++) {
      ...
      for (var j = 0, m = bnds.length; j < m; j++) {
        vertices = Q3D_air.Utils.arrayToVec3Array(bnds[j]);
        geom = Q3D_air.Utils.createWallGeometry(vertices, bzFunc, true);
        mesh = new THREE.Mesh(geom, material);
        mesh.name = "side";
        parent.add(mesh);
      }
      ...
      // bottom
      shape = new THREE.Shape(Q3D_air.Utils.arrayToVec2Array(bnds[0]));
      for (j = 1, m = bnds.length; j < m; j++) {
        shape.holes.push(new THREE.Path(Q3D_air.Utils.arrayToVec2Array(bnds[j])));
      }
...
Q3D_air.MapLayer
*/
Q3D_air.MapLayer = function () {
  this.properties = {};
  this.queryable = true;

  this.materials = new Q3D_air.Materials();
  this.materials.addEventListener("renderRequest", this.requestRender.bind(this));

  this.objectGroup = new Q3D_air.Group();
  this.queryObjs = [];
};

Q3D_air.MapLayer.prototype = Object.create(THREE.EventDispatcher.prototype);
Q3D_air.MapLayer.prototype.constructor = Q3D_air.MapLayer;

Q3D_air.MapLayer.prototype.addObject = function (object, queryable) {
...
Q3D_air.MapLayer.prototype.queryableObjects = function () {
  return (this.visible) ? this.queryObjs : [];
};

Q3D_air.MapLayer.prototype.removeAllObjects = function () {
...
Q3D_air.MapLayer.prototype.loadJSONObject = function (jsonObject, scene) {
  if (jsonObject.type == "layer") {
    // properties
    if (jsonObject.properties !== undefined) {
      this.properties = jsonObject.properties;
      this.visible = (jsonObject.properties.visible || Q3D_air.Config.allVisible) ? true : false;
    }
...
Object.defineProperty(Q3D_air.MapLayer.prototype, "opacity", {
  get: function () {
...
Object.defineProperty(Q3D_air.MapLayer.prototype, "visible", {
  get: function () {
...
Q3D_air.MapLayer.prototype.boundingBox = function (forceUpdate) {
...
Q3D_air.MapLayer.prototype.setWireframeMode = function (wireframe) {
  this.materials.setWireframeMode(wireframe);
};

Q3D_air.MapLayer.prototype.requestRender = function () {
  this.dispatchEvent({type: "renderRequest"});
};


/*
Q3D_air.DEMLayer --> Q3D_air.MapLayer
*/
Q3D_air.DEMLayer = function () {
  Q3D_air.MapLayer.call(this);
  this.type = Q3D_air.LayerType.DEM;
  this.blocks = [];
};

Q3D_air.DEMLayer.prototype = Object.create(Q3D_air.MapLayer.prototype);
Q3D_air.DEMLayer.prototype.constructor = Q3D_air.DEMLayer;

Q3D_air.DEMLayer.prototype.loadJSONObject = function (jsonObject, scene) {
  var old_shading = this.properties.shading;
  Q3D_air.MapLayer.prototype.loadJSONObject.call(this, jsonObject, scene);
  if (jsonObject.type == "layer") {
...
Q3D_air.DEMLayer.prototype.buildBlock = function (jsonObject, scene) {
  var _this = this;

  var index = jsonObject.block;
  this.blocks[index] = (jsonObject.grid !== undefined) ? (new Q3D_air.DEMBlock()) : (new Q3D_air.ClippedDEMBlock());
    ...
    if (jsonObject.sides || jsonObject.frame) {
      ...
      var buildSides = function () {
        ...
        if (jsonObject.sides) {
          ...
          _this.blocks[index].buildSides(_this, m, material.mtl, Q3D_air.Config.dem.side.bottomZ);
        }
        // build frame
        if (jsonObject.frame) {
          material = new Q3D_air.Material();
          ...
          _this.blocks[index].buildFrame(_this, m, material.mtl, Q3D_air.Config.dem.frame.bottomZ);
        }
        _this.requestRender();
      };
...
Q3D_air.DEMLayer.prototype.getZ = function (x, y) {
...
Q3D_air.DEMLayer.prototype.segmentizeLineString = function (lineString, zFunc) {
  ...
  for (var i = 1, l = lineString.length; i < l; i++) {
    ...
    for (var j = 0, m = p.length; j < m; j++) {
      ...
      // Q3D_air.Utils.putStick(x, y, zFunc);
  ...
  for (var i = 0, l = lineString.length - 1; i < l; i++) {
    Q3D_air.Utils.putStick(lineString[i][0], lineString[i][1], zFunc, 0.8);
  }
  */
...
Q3D_air.DEMLayer.prototype.setSideVisible = function (visible) {
...
/*
Q3D_air.VectorLayer --> Q3D_air.MapLayer
*/
Q3D_air.VectorLayer = function () {
  Q3D_air.MapLayer.call(this);
...
Q3D_air.VectorLayer.prototype = Object.create(Q3D_air.MapLayer.prototype);
Q3D_air.VectorLayer.prototype.constructor = Q3D_air.VectorLayer;

Q3D_air.VectorLayer.prototype.build = function (block) {};

Q3D_air.VectorLayer.prototype.clearLabels = function () {
...
Q3D_air.VectorLayer.prototype.buildLabels = function (features, getPointsFunc) {
  ...
  var line_mat = new THREE.LineBasicMaterial({color: Q3D_air.Config.label.connectorColor});
  ...
  for (var i = 0, l = features.length; i < l; i++) {
    ...
    getPointsFunc(f).forEach(function (pt) {
      ...
      if (Q3D_air.Config.label.queryable) {
        var obj = this.objectGroup.children[f.objIndices[0]];
        e.onclick = function () {
          appAIR.scene.remove(appAIR.queryMarker);
          appAIR.highlightFeature(obj);
          appAIR.render();
          appAIR.showQueryResult({x: pt[0], y: pt[1], z: pt[2]}, obj, true);
        };
...
Q3D_air.VectorLayer.prototype.loadJSONObject = function (jsonObject, scene) {
  Q3D_air.MapLayer.prototype.loadJSONObject.call(this, jsonObject, scene);
  if (jsonObject.type == "layer") {
    if (jsonObject.data !== undefined) {
      this.clearLabels();
      ...
      (jsonObject.data.blocks || []).forEach(function (block) {
        if (block.url !== undefined) Q3D_air.application.loadJSONFile(block.url);
        else {
...
Object.defineProperty(Q3D_air.VectorLayer.prototype, "visible", {
  get: function () {
    return Object.getOwnPropertyDescriptor(Q3D_air.MapLayer.prototype, "visible").get.call(this);
  },
  set: function (value) {
    ...
    Object.getOwnPropertyDescriptor(Q3D_air.MapLayer.prototype, "visible").set.call(this, value);
  }
});


/*
Q3D_air.PointLayer --> Q3D_air.VectorLayer
*/
Q3D_air.PointLayer = function () {
  Q3D_air.VectorLayer.call(this);
  this.type = Q3D_air.LayerType.Point;
};

Q3D_air.PointLayer.prototype = Object.create(Q3D_air.VectorLayer.prototype);
Q3D_air.PointLayer.prototype.constructor = Q3D_air.PointLayer;

Q3D_air.PointLayer.prototype.loadJSONObject = function (jsonObject, scene) {
  if (jsonObject.type == "layer" && jsonObject.properties.objType == "Model File" && jsonObject.data !== undefined) {
    if (this.models === undefined) {
      var _this = this;

      this.models = new Q3D_air.Models();
  ...
  Q3D_air.VectorLayer.prototype.loadJSONObject.call(this, jsonObject, scene);
};

Q3D_air.PointLayer.prototype.build = function (features) {
  ...
  if (objType == "Sphere") {
    ...
    setSR = function (mesh, geom) {
      ...
      mesh.rotateOnWorldAxis(Q3D_air.uv.i, -geom.d * deg2rad);
      mesh.rotateOnWorldAxis(Q3D_air.uv.k, -geom.dd * deg2rad);
    };
...
Q3D_air.PointLayer.prototype.buildPoints = function (features) {
  var f, geom, obj;
  for (var fidx = 0, flen = features.length; fidx < flen; fidx++) {
    f = features[fidx];
...
Q3D_air.PointLayer.prototype.buildIcons = function (features) {
...
Q3D_air.PointLayer.prototype.buildModels = function (features) {
  var _this = this,
...
Q3D_air.PointLayer.prototype.buildLabels = function (features) {
  Q3D_air.VectorLayer.prototype.buildLabels.call(this, features, function (f) { return f.geom.pts; });
};


/*
Q3D_air.LineLayer --> Q3D_air.VectorLayer
*/
Q3D_air.LineLayer = function () {
  Q3D_air.VectorLayer.call(this);
  this.type = Q3D_air.LayerType.Line;
};

Q3D_air.LineLayer.prototype = Object.create(Q3D_air.VectorLayer.prototype);
Q3D_air.LineLayer.prototype.constructor = Q3D_air.LineLayer;

Q3D_air.LineLayer.prototype.loadJSONObject = function (jsonObject, scene) {
  Q3D_air.VectorLayer.prototype.loadJSONObject.call(this, jsonObject, scene);
};

Q3D_air.LineLayer.prototype.build = function (features) {
  var createObject,
  ...
  if (objType == this._lastObjType && this._createObject !== undefined) {
    ...
    var mesh, pt0 = new THREE.Vector3(), pt1 = new THREE.Vector3(), sub = new THREE.Vector3(), axis = Q3D_air.uv.j;

    createObject = function (f, line) {
      var group = new Q3D_air.Group();
    ...
    createObject = function (f, line) {
      ...
      return new THREE.Mesh(Q3D_air.Utils.createWallGeometry(vertices, bzFunc),
                            materials.mtl(f.mtl));
    };
  }
...
Q3D_air.LineLayer.prototype.buildLabels = function (features) {
  // Line layer doesn't support label
  // Q3D_air.VectorLayer.prototype.buildLabels.call(this, features);
};


/*
Q3D_air.PolygonLayer --> Q3D_air.VectorLayer
*/
Q3D_air.PolygonLayer = function () {
  Q3D_air.VectorLayer.call(this);
  this.type = Q3D_air.LayerType.Polygon;
...
Q3D_air.PolygonLayer.prototype = Object.create(Q3D_air.VectorLayer.prototype);
Q3D_air.PolygonLayer.prototype.constructor = Q3D_air.PolygonLayer;

Q3D_air.PolygonLayer.prototype.loadJSONObject = function (jsonObject, scene) {
  Q3D_air.VectorLayer.prototype.loadJSONObject.call(this, jsonObject, scene);
};

Q3D_air.PolygonLayer.prototype.build = function (features) {
  var createObject,
      materials = this.materials;
  ...
  if (this.properties.objType == this._lastObjType && this._createObject !== undefined) {
    ...
    var createSubObject = function (f, polygon, z) {
      ...
      var shape = new THREE.Shape(Q3D_air.Utils.arrayToVec2Array(polygon[0]));
      for (i = 1, l = polygon.length; i < l; i++) {
        shape.holes.push(new THREE.Path(Q3D_air.Utils.arrayToVec2Array(polygon[i])));
      }
...
Q3D_air.PolygonLayer.prototype.buildLabels = function (features) {
  Q3D_air.VectorLayer.prototype.buildLabels.call(this, features, function (f) { return f.geom.centroids; });
};

Q3D_air.PolygonLayer.prototype.setBorderVisible = function (visible) {
...
Q3D_air.PolygonLayer.prototype.setSideVisible = function (visible) {
  if (this.properties.objType != "Overlay") return;
...
Q3D_air.Model
*/
Q3D_air.Model = function () {
  this.loaded = false;
};

Q3D_air.Model.prototype = {

  constructor: Q3D_air.Model,

  // callback is called when model has been completely loaded
  load: function (url, callback) {
    var _this = this;
    Q3D_air.application.loadModelFile(url, function (model) {
      _this.model = model;
      _this._loadCompleted(callback);
    });
  },
  ...
  loadData: function (data, ext, resourcePath, callback) {
    var _this = this;
    Q3D_air.application.loadModelData(data, ext, resourcePath, function (model) {
      _this.model = model;
      _this._loadCompleted(callback);
    });
  },
...
Q3D_air.Models
*/
Q3D_air.Models = function () {
  this.models = [];
  this.cache = {};
};

Q3D_air.Models.prototype = Object.create(THREE.EventDispatcher.prototype);
Q3D_air.Models.prototype.constructor = Q3D_air.Models;

Q3D_air.Models.prototype.loadJSONObject = function (jsonObject) {
  var _this = this;
...
Q3D_air.Models.prototype.get = function (index) {
  return this.models[index];
};

Q3D_air.Models.prototype.clear = function () {
  this.models = [];
};


// Q3D_air.Utils - Utilities
Q3D_air.Utils = {};

// Put a stick to given position (for debug)
Q3D_air.Utils.putStick = function (x, y, zFunc, h) {
  if (Q3D_air.Utils._stick_mat === undefined) Q3D_air.Utils._stick_mat = new THREE.LineBasicMaterial({color: 0xff0000});
  if (h === undefined) h = 0.2;
  if (zFunc === undefined) {
    zFunc = function (x, y) { return Q3D_air.application.scene.mapLayers[0].getZ(x, y); };
  }
  var z = zFunc(x, y);
  var geom = new THREE.Geometry();
  geom.vertices.push(new THREE.Vector3(x, y, z + h), new THREE.Vector3(x, y, z));
  var stick = new THREE.Line(geom, Q3D_air.Utils._stick_mat);
  Q3D_air.application.scene.add(stick);
};

// convert latitude and longitude in degrees to the following format
// Ndd°mm′ss.ss″, Eddd°mm′ss.ss″
Q3D_air.Utils.convertToDMS = function (lat, lon) {
...
Q3D_air.Utils.createWallGeometry = function (vertices, bzFunc, buffer_geom) {
  var geom = new THREE.Geometry(),
      pt = vertices[0];
  geom.vertices.push(
...
Q3D_air.Utils.arrayToVec2Array = function (points) {
  var pt, pts = [];
...
Q3D_air.Utils.arrayToVec3Array = function (points, zFunc) {
  var pt, pts = [];
  if (zFunc === undefined) {
    for (var i = 0, l = points.length; i < l; i++) {
      pt = points[i];
...
Q3D_air.Utils.arrayToFace3Array = function (faces) {
  var f, fs = [];
...
Q3D_air.Utils.setGeometryUVs = function (geom, base_width, base_height) {
  var face, v, uvs = [];
